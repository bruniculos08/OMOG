...
if __name__ == "__main__":
    ...
    Bspline_Points = [Point(0, 1.9, 0), Point(1, 1.5, 0), Point(2, 2, 0),
                        Point(3, 1.5, 0), Point(4, 0.5, 0), Point(5, -1.5, 0), 
                        Point(6, 9, 0), Point(7, 4, 0), Point(5, 3.8, 0)]
    ...
    Bezier_Points = [Point(1, 3, 0), Point(2, 2, 0), Point(1.25, 3, 0) , Point(2.5, 3.5, 0), 
                    Point(1.5, 5.5, 0), Point(4, -0.5, 0), Point(4, 0, 0), Point(5, 2, 0)]

    print("Aviso 1: os pontos pretos no gráfico se referem ao último ponto calculado da curva (derivada ou não) B-spline e ao primeiro ponto calculado da (derivada ou não) Bézier.")
    print("Aviso 2: a curva B-spline (e suas derivadas) são coloridas de acordo com os intervalos do vetor de nós enquanto a curva Bézier (e suas derivadas) tem cor verde.")

    print("Digite o código para a continuidade deseja: ")
    print("0 - C0 \n1 - C1 \n2 - C2")
    flagContinuity = int(input())
    ...
    if(flagCurve == 0):
        ...
        P_bezier = Plot_Bspline(Bspline_Points, D, T)
        P_bspline = Plot_Bezier(Bezier_Points, "green")
    
    if(flagCurve == 1):
        # (7) Gráficos das derivadas de primeira ordem:
        P_bezier = Plot_Bezier_kDerivative(Bezier_Points, "green", 1)
        P_bspline = Plot_Bspline_kDerivative(Bspline_Points, D, 1)

    if(flagCurve == 2):
        # (8) Gráficos das derivadas de segunda ordem:
        P_bezier = Plot_Bezier_kDerivative(Bezier_Points, "green", 2)
        P_bspline = Plot_Bspline_kDerivative(Bspline_Points, D, 2)

    # (9) Este bloco de código ajusta o limites dos eixos do gráfico para melhor visualização:
    P_max, P_min = get_MaxValues([P_bezier, P_bspline, Bspline_Points, Bezier_Points])
    plt.xlim(P_min.x - 1, P_max.x + 1)
    plt.ylim(P_min.y - 1, P_max.y + 1)

    plt.savefig("Exemplo-Join-Continuity-Bspline-Bezier.svg", format = 'svg')
    plt.savefig("Exemplo-Join-Continuity-Bspline-Bezier.png", format = 'png')
    plt.show()
    plt.close()